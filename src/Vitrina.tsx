...
export default function Vitrina() {
  ...
  const compartirProducto = async (producto: any, e?: any) => {
    if (e) e.stopPropagation();
    const url = window.location.href;
    const texto = `👋 Mira este producto: *${producto.nombre}* en ${tienda?.nombre_local || 'el catálogo'}`;
    if (navigator.share) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF8] font-sans text-stone-900 pb-10 relative">
      ...
      {fotoFullscreen && (
        ...
        >
          <button
            className="absolute top-4 right-4 text-white/70 hover:text-white text-3xl font-bold w-12 h-12 flex items-center justify-center"
            onClick={() => setFotoFullscreen(null)}
          >✕</button>
          <img
            src={fotoFullscreen}
      ...
      <main className="max-w-5xl mx-auto px-4">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 bg-white border border-stone-200 rounded-2xl text-sm font-medium text-stone-800 placeholder-stone-400 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-400 transition-all shadow-sm"
          />
          {busqueda && (
            <button
              onClick={() => setBusqueda('')}
              className="absolute right-4 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-600 font-bold text-xl leading-none"
            >×</button>
          )}
        </div>
        ...
        {busqueda && (
          <p className="text-xs text-stone-400 font-medium mb-4">
            {productosFiltradosPorBusqueda.length === 0
              ...
              : `${productosFiltradosPorBusqueda.length} resultado${productosFiltradosPorBusqueda.length !== 1 ? 's' : ''} para "${busqueda}"`}
          </p>
        )}

        {/* GRID */}
        {productosAgrupados.length === 0 || productosAgrupados.every(g => g.items.length === 0) ? (
          <div className="text-center bg-white p-8 rounded-3xl border border-stone-200 mt-4">
            <span className="text-4xl mb-3 block">🪑</span>
            <p className="text-stone-500 text-sm">
              {busqueda ? 'No encontramos productos con ese nombre.' : 'Aún no hay productos publicados en esta vitrina.'}
            </p>
          </div>
        ) : (
          ...
          <div className="bg-[#FDFCF8] w-full max-w-4xl sm:rounded-3xl rounded-t-3xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col sm:flex-row relative slide-up">
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    />
                    <div className="absolute bottom-3 right-3 bg-black/40 text-white text-[10px] px-2.5 py-1 rounded-full font-bold pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                      <span>🔍</span> Ampliar
                    </div>
                    {fotos.length > 1 && (
                      <>
                        <button onClick={() => setFotoActualIndex(prev => prev === 0 ? fotos.length - 1 : prev - 1)}
                        ...
                        ><span className="text-2xl font-bold leading-none mb-1">‹</span></button>
                        <button onClick={() => setFotoActualIndex(prev => prev === fotos.length - 1 ? 0 : prev + 1)}
            ...
            <div className="w-full sm:w-1/2 p-6 sm:p-10 flex flex-col bg-white overflow-y-auto max-h-[50vh] sm:max-h-full">
              ...
              <div className="flex flex-col gap-2 w-full mt-4">
                <button onClick={() => agregarACotizacion(productoSeleccionado)}
                ...
                <button onClick={() => consultarPorWhatsApp(productoSeleccionado)}
      ...
      {mostrarSubir && (
        <button
          ...
          className={`fixed z-40 bg-white border border-stone-200 text-stone-500 hover:text-amber-800 hover:border-amber-300 shadow-md w-11 h-11 rounded-full flex items-center justify-center text-lg font-bold transition-all hover:scale-105 active:scale-95 ${carrito.length > 0 ? 'bottom-24 right-6' : 'bottom-6 right-6'}`}
          title="Volver arriba"
        >↑</button>
      )}
          ...
          <div className="bg-white w-full max-w-lg sm:rounded-3xl rounded-t-3xl max-h-[90vh] flex flex-col relative slide-up shadow-2xl">
            ...
            {carrito.length > 0 && (
              <div className="p-6 border-t border-stone-100 bg-stone-50 sm:rounded-b-3xl">
                <button onClick={enviarCotizacionFinal}
