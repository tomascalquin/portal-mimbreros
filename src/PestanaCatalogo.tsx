...
export default function PestanaCatalogo({ miId, nombreLocal }: any) {
  ...
  // Guarda el ID de la categoría (UUID)
  ...
  const guardarProducto = async (e: any) => {
    ...
    const datos: any = { 
      ...
      categoria_id: prodCategoriaId || null // Se guarda el UUID
    };
  ...
  const abrirParaEditar = (p: any, e: any) => {
    ...
    setProdCategoriaId(p.categoria_id || ''); // Cargar el UUID
    setProdArchivo(null); 
  ...
  return (
    <>
      ...
      <div className="space-y-6 fade-in print:hidden">
        ...
        <button onClick={() => { setProductoAEditar(null); setProdNombre(''); setProdDesc(''); setProdPrecio(''); setProdStock('1'); setProdCategoriaId(''); setProdArchivo(null); setProdArchivo2(null); setModalProducto(true); }} className="w-full bg-amber-600 text-white p-4 rounded-xl font-bold shadow-md flex justify-center items-center gap-2 hover:bg-amber-700 transition-colors">
          <span className="text-xl leading-none">+</span> Nuevo Producto
        </button>

        <div className="grid gap-3 pb-10">
          {misProductos.length === 0 ? (
            <p className="text-stone-400 text-sm text-center py-4 bg-white rounded-xl border border-dashed border-stone-300">Aún no tienes productos en la vitrina.</p>
          ) : (
            misProductos.map(p => {
              // Buscar el nombre de la categoría cruzando con la lista de categorías
            ...
            <div className="bg-white w-full max-w-md p-6 rounded-t-3xl sm:rounded-2xl max-h-[90vh] overflow-y-auto slide-up">
              ...
              <form onSubmit={guardarProducto} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" required value={prodNombre} onChange={e => setProdNombre(e.target.value)} className="w-full p-3 border border-stone-300 rounded-lg focus:border-amber-600 focus:outline-none" />
                </div>

                {/* SELECTOR RELACIONAL DE CATEGORÍA */}
                <div>
                  ...
                  <select 
                    ...
                    className="w-full p-3 border border-stone-300 rounded-lg bg-white focus:border-amber-600 focus:outline-none"
                  >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" required value={prodPrecio} onChange={e => setProdPrecio(e.target.value)} className="w-full p-3 border border-stone-300 rounded-lg focus:border-amber-600 focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-stone-700 mb-1">Stock</label>
                    <input type="number" required value={prodStock} onChange={e => setProdStock(e.target.value)} className="w-full p-3 border border-stone-300 rounded-lg focus:border-amber-600 focus:outline-none" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-bold text-stone-700 mb-1">Descripción</label>
                  <textarea value={prodDesc} onChange={e => setProdDesc(e.target.value)} className="w-full p-3 border border-stone-300 rounded-lg h-20 focus:border-amber-600 focus:outline-none" />
                </div>
                <button type="submit" disabled={guardandoProd} className="w-full bg-amber-600 text-white py-3.5 rounded-xl font-bold shadow-md mt-2">
                  {guardandoProd ? 'Guardando...' : (productoAEditar ? 'Actualizar Producto' : 'Publicar')}
                </button>
              </form>
            </div>
          </div>
